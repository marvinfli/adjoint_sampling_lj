...
# modified from https://github.com/gcorso/torsional-diffusion/blob/master/diffusion/score_model.py
import math
...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(
        ...
        self.final_edge_embedding = nn.Sequential(
            nn.Linear(radius_embed_dim, ns), nn.ReLU(), nn.Linear(ns, ns)
        )
        ...
        self.final_linear = nn.Sequential(
            nn.Linear(ns, ns, bias=False), nn.Tanh(), nn.Linear(ns, 1, bias=False)
        )
    ...
    def forward(self, data):
        ...
        bond_vec = data.positions[bonds[1]] - data.positions[bonds[0]]
    ...
    def build_bond_conv_graph(self, data, node_attr):
        ...
        bond_pos = (data.positions[bonds[0]] + data.positions[bonds[1]]) / 2
        ...
        edge_vec = data.positions[edge_index[1]] - bond_pos[edge_index[0]]
    ...
    def build_conv_graph(self, data):
        ...
        radius_edges = radius_graph(data.positions, self.max_radius, data.batch)
        ...
        node_sigma_emb = get_timestep_embedding(
            node_sigma, self.sigma_embed_dim, dtype=next(self.parameters()).dtype
        )
        ...
        src, dst = edge_index
        edge_vec = data.positions[dst.long()] - data.positions[src.long()]
...
def get_timestep_embedding(
    timesteps, embedding_dim, dtype: torch.dtype, max_positions=10000
):
